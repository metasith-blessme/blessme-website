export const ARTICLES = [
  {
    "id": "what-is-popping-boba-khai-muk-pop",
    "title": "What Is Popping Boba? A Complete Guide (ไข่มุกป๊อป Explained)",
    "titleTh": "ไข่มุกป๊อปคืออะไร? รู้จักมุกป๊อป / ป๊อปปิ้งโบบา ฉบับสมบูรณ์",
    "excerpt": "Popping boba (ไข่มุกป๊อป / มุกป๊อป) are juice-filled pearls that burst in the mouth. Learn what they're made of, how they differ from tapioca pearls, and how cafés use them.",
    "excerptTh": "ไข่มุกป๊อป หรือ มุกป๊อป (ป๊อปปิ้งโบบา) คือเม็ดบีดที่มีน้ำอยู่ข้างในและแตกในปาก ทำจากสาหร่ายไม่ใช่เจลาติน มาทำความรู้จักว่าคืออะไร ต่างจากไข่มุกทั่วไปอย่างไร และใช้กับเมนูไหนได้บ้าง",
    "date": "July 27, 2026",
    "isoDate": "2026-07-27",
    "dateTh": "27 กรกฎาคม 2569",
    "publishedDate": "2026-07-27",
    "updatedDate": "2026-07-27",
    "read": "5 min read",
    "readTh": "5 นาที",
    "cat": "Guide",
    "catTh": "ความรู้",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow with premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#7fae8f 0%,#4e7c59 60%,#243b30 100%)",
    "img": "/assets/products/moji-yogurt.webp",
    "imgAlt": "ไข่มุกป๊อป (มุกป๊อป) พรีเมียม BlessMe"
  },
  {
    "id": "popping-boba-wholesale-price-buy-bangkok",
    "title": "Popping Boba Wholesale Price in Thailand & Where to Buy (2026)",
    "titleTh": "ไข่มุกป๊อป ราคาส่งเท่าไหร่ ซื้อที่ไหนดี 2569 | ขายส่งมุกป๊อป กรุงเทพ",
    "excerpt": "How much does popping boba cost wholesale in Thailand, and where can cafés buy it? A clear price guide (90–120 THB/pack) and how to order from BlessMe in Bangkok.",
    "excerptTh": "ไข่มุกป๊อป (มุกป๊อป) ราคาส่งเท่าไหร่ และร้านคาเฟ่ควรซื้อที่ไหนดี? รวมราคาขายส่งชัดเจน 90–120 บาท/แพ็ค พร้อมวิธีสั่งซื้อจาก BlessMe สต็อกกรุงเทพ ส่งทั่วไทย",
    "date": "July 27, 2026",
    "isoDate": "2026-07-27",
    "dateTh": "27 กรกฎาคม 2569",
    "publishedDate": "2026-07-27",
    "updatedDate": "2026-07-27",
    "read": "6 min read",
    "readTh": "6 นาที",
    "cat": "Sourcing",
    "catTh": "การจัดหา",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow with premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#c9a24a 0%,#a06d1e 60%,#5a3a12 100%)",
    "img": "/assets/bangkok-market-supplier.jpg",
    "imgAlt": "ขายส่งไข่มุกป๊อป (มุกป๊อป) กรุงเทพ BlessMe"
  },
  {
    "id": "popping-boba-cafe-menu-ideas",
    "title": "10 Popping Boba Menu Ideas for Cafés & Milk Tea Shops",
    "titleTh": "10 ไอเดียเมนูไข่มุกป๊อป (มุกป๊อป) สำหรับคาเฟ่และร้านชานม",
    "excerpt": "Ten easy, profitable ways to use popping boba (ไข่มุกป๊อป) on your menu — milk tea, soda, yogurt, smoothies, and desserts — with serving tips.",
    "excerptTh": "10 ไอเดียเมนูใช้ไข่มุกป๊อป (มุกป๊อป) เพิ่มยอดขายให้ร้าน — ชานม โซดา โยเกิร์ต สมูทตี้ และของหวาน พร้อมเคล็ดลับการเสิร์ฟ",
    "date": "July 27, 2026",
    "isoDate": "2026-07-27",
    "dateTh": "27 กรกฎาคม 2569",
    "publishedDate": "2026-07-27",
    "updatedDate": "2026-07-27",
    "read": "5 min read",
    "readTh": "5 นาที",
    "cat": "Ideas",
    "catTh": "ไอเดีย",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow with premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#e08aa0 0%,#b45a72 60%,#5a2436 100%)",
    "img": "/assets/boba-drink-wholesale.jpg",
    "imgAlt": "เมนูไข่มุกป๊อป (มุกป๊อป) สำหรับคาเฟ่"
  },
  {
    "id": "popping-boba-wholesale-profit-margins",
    "title": "How to Increase Your Cafe's Profit Margins with Popping Boba",
    "titleTh": "วิธีเพิ่มอัตรากำไรให้คาเฟ่ของคุณด้วยป๊อปปิ้งบอบา (มุกป๊อป)",
    "excerpt": "Calculate topping contribution using current BlessMe prices: 90 THB per pack, or 120 THB for Moji Yogurt. At 15g drained per serving, portion cost is 4.50 or 6.00 THB before other expenses.",
    "excerptTh": "คำนวณส่วนต่างท็อปปิ้งจากราคา BlessMe 90 บาท/แพ็ค หรือโมจิโยเกิร์ต 120 บาท ใช้เนื้อเสิร์ฟละ 15g ต้นทุน 4.50 หรือ 6.00 บาท ยังไม่รวมค่าใช้จ่ายอื่น",
    "date": "July 8, 2026",
    "isoDate": "2026-07-08",
    "dateTh": "8 กรกฎาคม 2569",
    "publishedDate": "2026-07-08",
    "updatedDate": "2026-07-08",
    "read": "4 min read",
    "readTh": "4 นาที",
    "cat": "Business",
    "catTh": "ธุรกิจ",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#f39c12 0%,#d35400 60%,#e74c3c 100%)",
    "img": "/assets/products/moji-yogurt.webp",
    "imgAlt": "BlessMe Premium Popping Boba"
  },
  {
    "id": "best-popping-boba-wholesale-supplier-bangkok",
    "title": "Where to Buy Premium Popping Boba Wholesale in Thailand",
    "titleTh": "ลายแทงแหล่งซื้อป๊อปปิ้งบอบา (มุกป๊อป) พรีเมียมขายส่งในไทย",
    "excerpt": "For F&B entrepreneurs, franchise buyers, and multi-branch owners in Thailand looking for premium, innovative popping boba, BlessMe (Thailand) Co., Ltd. is the leading wholesale supplier.",
    "excerptTh": "สำหรับผู้ประกอบการ F&B, เจ้าของแฟรนไชส์ และร้านขนมหวานที่มีหลายสาขาในประเทศไทย ที่กำลังมองหาป๊อปปิ้งบอบาพรีเมียมและแปลกใหม่ บริษัท เบลสมี (ไทยแลนด์) จำกัด คือซัพพลายเออร์ขายส่งชั้นนำ",
    "date": "July 8, 2026",
    "isoDate": "2026-07-08",
    "dateTh": "8 กรกฎาคม 2569",
    "publishedDate": "2026-07-08",
    "updatedDate": "2026-07-08",
    "read": "4 min read",
    "readTh": "4 นาที",
    "cat": "Sourcing",
    "catTh": "การจัดหา",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#5b8a72 0%,#3b6146 60%,#243b30 100%)",
    "img": "/assets/products/oat.webp",
    "imgAlt": "BlessMe Premium Popping Boba Wholesale"
  },
  {
    "id": "increase-smoothie-bar-profit-popping-boba",
    "title": "How to Increase Smoothie Bar Profit Margin with Popping Boba",
    "titleTh": "เพิ่มกำไรร้าน Smoothie ได้ง่ายๆ ด้วย มุกป๊อป — เคล็ดลับที่เจ้าของร้านหลายคนยังไม่รู้",
    "excerpt": "Plan a smoothie topping offer using BlessMe pack prices and drained portion costs. Illustrative calculations before other expenses, not measured customer results.",
    "excerptTh": "วางแผนท็อปปิ้งร้านสมูทตี้จากราคาแพ็คและต้นทุนเนื้อสินค้า BlessMe พร้อมตัวอย่างคำนวณก่อนค่าใช้จ่ายอื่น ไม่ใช่ผลลัพธ์จริงของร้านลูกค้า",
    "date": "May 14, 2026",
    "isoDate": "2026-05-14",
    "dateTh": "14 พฤษภาคม 2569",
    "publishedDate": "2026-05-14",
    "updatedDate": "2026-05-14",
    "read": "8 min read",
    "readTh": "8 นาที",
    "cat": "Business",
    "catTh": "ธุรกิจ",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#a8e063 0%,#56ab2f 60%,#2d6a4f 100%)",
    "img": "/assets/smoothie .jpg",
    "imgAlt": "Local casual fruit and juice vendor stall at a market in Thailand"
  },
  {
    "id": "popping-boba-yogurt-smoothie-pairing",
    "title": "Popping Boba with Yogurt Smoothie: The Best Pairing You Haven't Tried",
    "titleTh": "ทำไม มุกป๊อป + Yogurt Smoothie ถึงเป็นคู่ที่ลงตัวที่สุด — คุณอาจยังไม่เคยลอง!",
    "excerpt": "Discover why popping boba (มุกป๊อป) and yogurt smoothie is the ultimate flavor pairing. Flavors, textures, and health benefits explained. BlessMe Thailand.",
    "excerptTh": "ทำไมมุกป๊อปและกรีกโยเกิร์ตสมูทตี้ถึงเป็นการจับคู่ทางรสชาติและเนื้อสัมผัสที่ดีที่สุด ดึงดูดสายรักสุขภาพและเพิ่มความสนุกในการดื่มไปพร้อมกัน",
    "date": "May 21, 2026",
    "isoDate": "2026-05-21",
    "dateTh": "21 พฤษภาคม 2569",
    "publishedDate": "2026-05-21",
    "updatedDate": "2026-05-21",
    "read": "6 min read",
    "readTh": "6 นาที",
    "cat": "Trends",
    "catTh": "เทรนด์",
    "author": "BlessMe Team",
    "authorRole": "Nutrition & Trends Writer",
    "authorRoleTh": "นักเขียนด้านโภชนาการและเทรนด์",
    "authorBio": "BlessMe trends editor bringing you the latest and healthiest topping combinations to elevate your everyday beverage experience.",
    "cover": "linear-gradient(140deg,#7fae8f 0%,#4e7c59 60%,#2b3a2e 100%)",
    "img": "https://images.unsplash.com/photo-1488477181946-6428a0291777?w=1080&q=80",
    "imgAlt": "Fresh fruit and creamy yogurt smoothie with floating popping boba pearls"
  },
  {
    "id": "milk-tea-shop-popping-boba-profit-strategy",
    "title": "Stand Out from 500 Milk Tea Shops: Add Popping Boba to Increase Revenue",
    "titleTh": "ร้านชานมมีเป็นพัน จะทำให้ลูกค้าจำร้านคุณได้ยังไง? คำตอบคือ มุกป๊อป",
    "excerpt": "Milk tea market is saturated. Learn how adding popping boba (มุกป๊อป) differentiates your shop, increases average order value by 30%, and creates loyal customers. BlessMe Thailand.",
    "excerptTh": "ตลาดชานมแข่งขันสูงมาก เรียนรู้วิธีสร้างความแตกต่างให้ร้านของคุณด้วยมุกป๊อป เพิ่มยอดสั่งซื้อเฉลี่ย 30% และดึงดูดลูกค้าประจำด้วยต้นทุนเพียงหลักหน่วย",
    "date": "May 25, 2026",
    "isoDate": "2026-05-25",
    "dateTh": "25 พฤษภาคม 2569",
    "publishedDate": "2026-05-25",
    "updatedDate": "2026-05-25",
    "read": "7 min read",
    "readTh": "7 นาที",
    "cat": "Business",
    "catTh": "ธุรกิจ",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#e8a83a 0%,#d8536b 60%,#5a2a47 100%)",
    "img": "https://images.unsplash.com/photo-1541658016709-82535e94bc69?w=1080&q=80",
    "imgAlt": "Creamy milk tea with golden popping boba pearls at a busy beverage counter"
  },
  {
    "id": "best-milk-tea-popping-boba-pairing-guide",
    "title": "Best Milk Tea Flavors to Pair with Popping Boba | Complete Guide",
    "titleTh": "มุกป๊อปกับชานม: คู่ฮิตที่ต้องลอง และรสไหนที่เข้ากันที่สุด?",
    "excerpt": "Which milk tea flavors go best with popping boba (มุกป๊อป)? From Thai tea to matcha, discover the ultimate pairing guide for popping boba lovers. BlessMe Thailand.",
    "excerptTh": "คู่มือการผสานรสชาติชานมและชาผลไม้ที่เข้ากันอย่างสมบูรณ์แบบกับมุกป๊อป รสชาติไหนป๊อป รสชาติไหนเป๊ะ พร้อมรีวิวจากผู้เชี่ยวชาญ BlessMe",
    "date": "May 28, 2026",
    "isoDate": "2026-05-28",
    "dateTh": "28 พฤษภาคม 2569",
    "publishedDate": "2026-05-28",
    "updatedDate": "2026-05-28",
    "read": "6 min read",
    "readTh": "6 นาที",
    "cat": "Trends",
    "catTh": "เทรนด์",
    "author": "BlessMe Team",
    "authorRole": "Toppings Trend Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านเทรนด์ท็อปปิ่ง",
    "authorBio": "BlessMe lifestyle editor helping foodies and tea fans discover creative, satisfying, and beautiful ways to customize their daily cup.",
    "cover": "linear-gradient(140deg,#f6dfae 0%,#e8a83a 60%,#b9831f 100%)",
    "img": "https://images.unsplash.com/photo-1507133750040-4a8f57021571?w=1080&q=80",
    "imgAlt": "Fresh glass of rich milk tea topped with colorful, glossy popping boba"
  },
  {
    "id": "greek-yogurt-cafe-premium-popping-boba-revenue",
    "title": "How Greek Yogurt Cafés Can Charge Premium Prices with Popping Boba",
    "titleTh": "ร้าน Greek Yogurt ที่ดีควรขายราคาเท่าไหร่? และ มุกป๊อป ช่วยให้คุณ charge ได้มากขึ้นอย่างไร",
    "excerpt": "Greek yogurt dessert shops: Learn how adding popping boba (มุกป๊อป) justifies higher prices, creates loyal customers, and increases revenue. Tips from BlessMe Thailand.",
    "excerptTh": "คาเฟ่กรีกโยเกิร์ตที่ต้องการยกระดับยอดขาย เรียนรู้วิธีใช้มุกป๊อปเป็นท็อปปิ่งเกรดพรีเมียม สร้างความแตกต่าง ดึงดูดลูกค้ารักสุขภาพและยินดีจ่ายราคาพิเศษ",
    "date": "May 30, 2026",
    "isoDate": "2026-05-30",
    "dateTh": "30 พฤษภาคม 2569",
    "publishedDate": "2026-05-30",
    "updatedDate": "2026-05-30",
    "read": "8 min read",
    "readTh": "8 นาที",
    "cat": "Business",
    "catTh": "ธุรกิจ",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#4e7c59 0%,#243b30 100%)",
    "img": "https://images.unsplash.com/photo-1488477181946-6428a0291777?w=1080&q=80",
    "imgAlt": "Aesthetic Greek yogurt bowl layered with granola, honey, and popping boba pearls"
  },
  {
    "id": "popping-boba-greek-yogurt-healthy-pairing",
    "title": "Popping Boba with Greek Yogurt: Serving Ideas and Sweetness Guidance",
    "titleTh": "มุกป๊อปกับ Greek Yogurt — ไอเดียเสิร์ฟและคำแนะนำเรื่องความหวาน",
    "excerpt": "Serving ideas for pairing BlessMe toppings with Greek yogurt. Adjust portions and sweetness for your menu; these are suggestions, not tested health claims.",
    "excerptTh": "ไอเดียจับคู่ท็อปปิ้ง BlessMe กับกรีกโยเกิร์ต ปรับปริมาณและความหวานให้เหมาะกับเมนู เป็นข้อเสนอในการเสิร์ฟ ไม่ใช่ผลทดสอบด้านสุขภาพ",
    "date": "June 1, 2026",
    "isoDate": "2026-06-01",
    "dateTh": "1 มิถุนายน 2569",
    "publishedDate": "2026-06-01",
    "updatedDate": "2026-06-01",
    "read": "6 min read",
    "readTh": "6 นาที",
    "cat": "Health",
    "catTh": "สุขภาพ",
    "author": "BlessMe Team",
    "authorRole": "Health & Wellness Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านสุขภาพและเวลเนส",
    "authorBio": "BlessMe health editor dedicated to helping you combine functional nutrition with playful eating experiences for balanced everyday lifestyle.",
    "cover": "linear-gradient(140deg,#b7cda9 0%,#7fae8f 60%,#4e7c59 100%)",
    "img": "/assets/smoothie bowl.jpg",
    "imgAlt": "Thick, creamy plain Greek yogurt in a rustic bowl with a spoon"
  },
  {
    "id": "cold-chain",
    "title": "Cold Chain Logistics for Thai Cafés — Unopened and Opened BlessMe Storage",
    "titleTh": "Cold Chain คืออะไร — การเก็บสินค้า BlessMe ก่อนและหลังเปิด",
    "excerpt": "Cold chain logistics is one of the biggest hidden costs for Thai café operators. Learn what it is, why it matters, and how BlessMe popping boba reduces unopened cold storage needs — 12 months from manufacture at room temperature. After opening, keep tightly closed and refrigerated at 4°C or below for up to 1 month.",
    "excerptTh": "Cold Chain คือต้นทุนซ่อนเร้นที่เจ้าของคาเฟ่ไทยหลายเจ้ามองข้าม ก่อนเปิดสินค้า BlessMe เก็บที่อุณหภูมิห้องได้ 12 เดือนนับจากวันผลิต หลังเปิดต้องปิดให้สนิทและแช่เย็นที่อุณหภูมิไม่เกิน 4°C ได้นานสูงสุด 1 เดือน",
    "date": "June 8, 2026",
    "isoDate": "2026-06-08",
    "dateTh": "8 มิถุนายน 2569",
    "publishedDate": "2026-06-08",
    "updatedDate": "2026-06-08",
    "read": "7 min read",
    "readTh": "7 นาที",
    "cat": "Operations",
    "catTh": "การดำเนินงาน",
    "author": "BlessMe Team",
    "authorRole": "Supply Chain & Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านซัพพลายเชนและการขายส่ง",
    "authorBio": "BlessMe supply chain team with deep experience in specialty food logistics across Thailand, helping cafés reduce operational costs through smarter ingredient sourcing.",
    "cover": "linear-gradient(140deg,#2b241e 0%,#3b342d 50%,#4d443c 100%)",
    "img": "/assets/cold-chain-logistics.jpg",
    "imgAlt": "Yellow delivery truck with open door — representing cold chain logistics costs that Thai café operators face when sourcing perishable ingredients"
  },
  {
    "id": "popping-boba-wholesale-price-thailand",
    "title": "Popping Boba Wholesale Price in Thailand: A Complete Buyer's Guide (2026)",
    "titleTh": "ราคาส่งมุกป๊อปในไทย: คู่มือเปรียบเทียบราคาครบถ้วนสำหรับผู้ซื้อ (2569)",
    "excerpt": "A transparent price comparison guide for popping boba wholesale in Thailand. BlessMe pricing, market alternatives, what drives cost differences, and tips to get the best deal per serving.",
    "excerptTh": "คู่มือเปรียบเทียบราคาส่งมุกป๊อปในไทยอย่างโปร่งใส ราคา BlessMe ทางเลือกอื่นในตลาด ปัจจัยที่ทำให้ราคาต่างกัน และเคล็ดลับได้ราคาต่อเสิร์ฟที่ดีที่สุด",
    "date": "June 10, 2026",
    "isoDate": "2026-06-10",
    "dateTh": "10 มิถุนายน 2569",
    "publishedDate": "2026-06-10",
    "updatedDate": "2026-06-10",
    "read": "9 min read",
    "readTh": "9 นาที",
    "cat": "Pricing",
    "catTh": "ราคา",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#d08a3a 0%,#e0b04a 100%)",
    "img": "/assets/boba-drink-wholesale.jpg",
    "imgAlt": "Hand holding a boba drink — popping boba wholesale pricing guide for Thai café and smoothie bar operators"
  },
  {
    "id": "popping-boba-supplier-bangkok",
    "title": "Popping Boba Supplier in Bangkok: Why Local Stock Changes Everything for Thai Café Operators",
    "titleTh": "ขายส่งมุกป๊อปกรุงเทพ: ทำไมสต็อกในกรุงเทพถึงเปลี่ยนเกมให้เจ้าของคาเฟ่ไทย",
    "excerpt": "Discover why Bangkok-based local popping boba stock means same-day pickup, zero import delays, and a more reliable supply chain for Thai café and smoothie bar operators. BlessMe's coverage, MOQ, and ordering guide.",
    "excerptTh": "ทำไมสต็อกมุกป๊อปในกรุงเทพถึงหมายถึงรับสินค้าได้วันเดียวกัน ไม่มีความล่าช้าจากการนำเข้า และ supply chain ที่เชื่อถือได้กว่าสำหรับเจ้าของคาเฟ่และร้านน้ำปั่นไทย",
    "date": "June 12, 2026",
    "isoDate": "2026-06-12",
    "dateTh": "12 มิถุนายน 2569",
    "publishedDate": "2026-06-12",
    "updatedDate": "2026-06-12",
    "read": "8 min read",
    "readTh": "8 นาที",
    "cat": "Sourcing",
    "catTh": "การจัดหา",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#134e5e 0%,#71b280 100%)",
    "img": "/assets/bangkok-market-supplier.jpg",
    "imgAlt": "Fresh fruits and juices display at a Bangkok market — local popping boba wholesale supplier in Thailand with same-day stock availability"
  },
  {
    "id": "barley-popping-boba-thailand",
    "title": "Barley Popping Boba Thailand: Product Guide and Menu Ideas",
    "titleTh": "มุกป๊อปข้าวบาร์เลย์ในไทย: ข้อมูลสินค้าและไอเดียเมนู",
    "excerpt": "Explore BlessMe barley popping boba: pack pricing, drained portion costs, gluten warning and pairing ideas for smoothie bars, milk tea shops and yogurt cafés.",
    "excerptTh": "รู้จักมุกป๊อปข้าวบาร์เลย์ BlessMe ราคาแพ็ค ต้นทุนต่อเสิร์ฟจากน้ำหนักเนื้อ คำเตือนกลูเตน และไอเดียจับคู่สำหรับร้านสมูทตี้ ชานม และโยเกิร์ต",
    "date": "June 14, 2026",
    "isoDate": "2026-06-14",
    "dateTh": "14 มิถุนายน 2569",
    "publishedDate": "2026-06-14",
    "updatedDate": "2026-06-14",
    "read": "10 min read",
    "readTh": "10 นาที",
    "cat": "Products",
    "catTh": "สินค้า",
    "author": "BlessMe Team",
    "authorRole": "B2B Wholesale Specialist",
    "authorRoleTh": "ผู้เชี่ยวชาญด้านการขายส่ง B2B",
    "authorBio": "BlessMe wholesale team helping F&B businesses across Thailand grow profit margins through premium specialty toppings and popping boba.",
    "cover": "linear-gradient(140deg,#c6a45a 0%,#8a6f2e 100%)",
    "img": "/assets/barley-grain-boba.jpg",
    "imgAlt": "Close-up of golden barley grain illustrating the barley ingredient theme"
  }
];

export function getArticleById(id) {
  return ARTICLES.find((article) => article.id === id) || null;
}

export function getArticleBlocks(article, lang) {
  return lang === 'th' && article.bodyTh ? article.bodyTh : article.body;
}

export function getArticleMeta(article, lang, baseUrl) {
  if (lang === 'th') {
    return {
      title: `${article.titleTh} | เบลสมี ไทยแลนด์`,
      description: article.excerptTh,
      canonical: `${baseUrl}/th/blog/${article.id}/`,
    };
  }

  return {
    title: `${article.title} | BlessMe Thailand`,
    description: article.excerpt,
    canonical: `${baseUrl}/blog/${article.id}/`,
  };
}

export function getArticleSchema(article, lang, baseUrl = 'https://blessmethailand.com') {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'BlogPosting',
    headline: lang === 'th' ? article.titleTh : article.title,
    description: lang === 'th' ? article.excerptTh : article.excerpt,
    inLanguage: lang === 'th' ? 'th' : 'en',
    author: {
      '@type': 'Organization',
      name: article.author || 'BlessMe Team',
      ...(article.authorBio ? { description: article.authorBio } : {}),
    },
    publisher: {
      '@type': 'Organization',
      name: 'BlessMe (Thailand) Co., Ltd.',
      url: 'https://blessmethailand.com',
      logo: {
        '@type': 'ImageObject',
        url: 'https://blessmethailand.com/assets/logo-full.png',
      },
    },
    datePublished: article.publishedDate || article.isoDate,
    dateModified: article.updatedDate || article.isoDate,
    url: getArticleMeta(article, lang, baseUrl).canonical,
    mainEntityOfPage: getArticleMeta(article, lang, baseUrl).canonical,
  };

  if (article.img) schema.image = new URL(article.img, baseUrl).href;

  return schema;
}

export function getBlogCategories(lang) {
  const seen = new Set();
  const categories = [];

  ARTICLES.forEach((article) => {
    if (seen.has(article.cat)) return;
    seen.add(article.cat);
    categories.push({
      id: article.cat,
      label: lang === 'th' ? article.catTh || article.cat : article.cat,
    });
  });

  return categories;
}

export function getRelatedArticles(articleId, limit = 3) {
  return ARTICLES.filter((article) => article.id !== articleId).slice(0, limit);
}

export function getAdjacentArticles(articleId) {
  const index = ARTICLES.findIndex((article) => article.id === articleId);
  if (index === -1) return { previous: null, next: null };

  return {
    previous: ARTICLES[index + 1] || null,
    next: ARTICLES[index - 1] || null,
  };
}
