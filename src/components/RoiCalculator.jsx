...
export default function RoiCalculator({ lang }) {
  ...
  const costPerServing = 4.50;
  const netProfitPerCup = Math.max(0, upcharge - costPerServing);
  const monthlyCups = dailyCups * 30;
  const monthlyProfit = Math.round(monthlyCups * netProfitPerCup);
  const annualProfit = Math.round(dailyCups * 365 * netProfitPerCup);
  const packsNeeded = Math.ceil(monthlyCups / 20);
  const marginPercent = Math.round((netProfitPerCup / upcharge) * 100);

  const lineQuoteMsg = isTh
    ? `สวัสดีครับ สนใจสั่งซื้อ BlessMe Popping Boba ประมาณเดือนละ ${packsNeeded} แพ็ค (สำหรับเสิร์ฟ ${monthlyCups} แก้ว/เดือน) รบกวนขอใบเสนอราคาพิเศษครับ`
    : `Hello, I would like to enquire about wholesale pricing for ~${packsNeeded} packs/month of BlessMe Popping Boba (~${monthlyCups} cups/month).`;
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col">
          ...
          <p className="text-white/70 text-sm sm:text-lg mb-6 sm:mb-8 font-normal leading-relaxed">
            {isTh 
              ? 'ด้วยต้นทุนต่อเสิร์ฟเพียง ~4.50฿ การเพิ่มเมนูป็อปปิ่งโบบาซิกเนเจอร์สามารถสร้างรายได้และกำไรสุทธิใหม่ให้ร้านคุณทันที'
              : 'With a low portion cost of ~4.50฿, adding signature popping boba creates a high-margin revenue stream for your café.'}
          </p>

          {/* Slider 1: Daily Cups */}
          <div className="mb-5 sm:mb-7 bg-white/5 p-4 sm:p-6 rounded-2xl border border-white/10">
            <div className="flex justify-between items-baseline mb-3">
              <label className="text-xs sm:text-base font-semibold text-white/90">
        ...
        <div className="lg:col-span-5 bg-[#1C3223] rounded-[24px] sm:rounded-[28px] p-6 sm:p-9 border border-[#3B6146] shadow-xl flex flex-col justify-between">
          ...
          <div>
            <div className="text-xs font-semibold text-white/60 uppercase tracking-wider mb-1">
              {isTh ? 'ประมาณการกำไรสุทธิส่วนเพิ่ม' : 'Estimated Net Profit Uplift'}
            </div>
            
            {/* Big Monthly Number */}
            <div className="my-3">
              <div className="text-xs text-[#E5C378] font-medium">{isTh ? 'กำไรเพิ่มต่อเดือน (30 วัน)' : 'Monthly Profit (30 Days)'}</div>
            ...
            <div className="pb-4 mb-5 border-b border-white/10">
              <span className="text-xs text-white/60">{isTh ? 'กำไรเพิ่มต่อปี: ' : 'Annual Net Uplift: '}</span>
              <strong className="text-sm text-[#E5C378] font-semibold">+฿{annualProfit.toLocaleString()} / {isTh ? 'ปี' : 'yr'}</strong>
            </div>

            {/* Breakdown Metrics */}
            <div className="space-y-3 text-xs sm:text-sm">
              <div className="flex justify-between text-white/80">
                <span>{isTh ? 'ต้นทุนต่อเสิร์ฟ (25g):' : 'Portion Cost (25g):'}</span>
                <span className="font-semibold text-white">~฿{costPerServing.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-white/80">
                <span>{isTh ? 'กำไรสุทธิต่อแก้ว:' : 'Net Profit per Cup:'}</span>
                <span className="font-semibold text-[#E5C378]">+฿{netProfitPerCup.toFixed(2)} ({marginPercent}% Margin)</span>
              </div>
              <div className="flex justify-between text-white/80">
